...
var looper = function(family){
  for(var i = 0; i < family.length; i++){alert(family[i]);}
};
...
var reversedLooper = function(letters){
  letters.reverse();
  for(var i = 0; i < letters.length; i++){alert(letters[i]);}
};
...
var divider = function(nums, evens, odds){
  var items = [];
  for(var i = 0; i < nums.length, i++){
    if(nums[i] % 2 === 0) evens[i].push(items[i]);
    else odds[i].push(items[i]);
  }
  items.push(evens, odds);
  return items;
};
...
var longest = function(a, b){
  var aLength = 0;
  var bLength = 0;
  for(var i = 0; i < a.length; i++){aLength++;}
  for(var i = 0; i < b.length; i++){bLength++}
